...
class TriggerEngine:
    ...
    def _restore_pending(self):
        for job in self.jobs.values():
            ...
            interrupted = state.last_started is not None and (
    ...
    def evaluate_job(self, job):
        ...
        if job.every is not None:
            if state.last_started is None or now - state.last_started >= job.every:
    ...
    def evaluate(self):
        ...
        return {job.identity: self.evaluate_job(job) for job in self.jobs.values()}
