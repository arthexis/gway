    ...
    def _job(self, job, project=None):
        ...
        if project is not None:
            try:
                ...
                raise LookupError(f"Unknown Sous Chef job {project!r}/{job!r}") from exc

        matches = [current for current in jobs.values() if current.name == job]
        if not matches:
            raise LookupError(f"Unknown Sous Chef job {job!r}")
        if len(matches) > 1:
            owners = ", ".join(sorted(current.project for current in matches))
            raise LookupError(
                f"Ambiguous Sous Chef job {job!r}; specify --project ({owners})"
            )
        return matches[0]
