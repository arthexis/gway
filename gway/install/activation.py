        ...
        for command in all_commands:
            ...
            if command not in desired:
                if final.exists() or final.is_symlink():
                    ...
                    backup = paths.bin / (f".{command}.gway-backup-{uuid.uuid4().hex}")
            ...
            if final.exists() or final.is_symlink():
                allowed = (
                    old_owned and _owned_launcher(final, project, command)
    ...
    try:
        for command in sorted(previous):
            ...
            if not final.exists() and not final.is_symlink():
                continue
            if not _owned_launcher(final, project, command):
                raise RuntimeError(f"Refusing to remove unmanaged launcher: {final}")
            backup = paths.bin / (f".{command}.gway-backup-{uuid.uuid4().hex}")
            os.replace(final, backup)
            backups.append((final, backup))
