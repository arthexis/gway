...
def discover_managed_projects(runtime):
    ...
    runtime._script_owners = {
        command: tuple(owners) for command, owners in script_owners.items()
    }
    runtime._main_owners = {
        package: tuple(owners) for package, owners in main_owners.items()
    }
...
def bootstrap(runtime, *, start=None):
    ...
    if isinstance(project_name, str) and project_name.strip():
        ...
        runtime._script_owners = {
            command: tuple(projects) for command, projects in owners.items()
        }
