...
- ``purge`` — Clears clipboard and optionally wipes history file.

  > ``gway clip purge``

- ``render_history_view`` — (no description)

  > ``gway clip render-history-view``

- ``track_history`` — Tracks clipboard history by polling at regular intervals.
...
- ``timestamp`` — Return the current timestamp in ISO-8601 format.
...
- ``setup_csms_v01_app`` — OCPP 1.6 CSMS implementation with RFID authorization.

  > ``gway ocpp setup-csms-v01-app``

- ``setup_csms_v16_app`` — Minimal OCPP 1.6 CSMS implementation for conformance testing.

  > ``gway ocpp setup-csms-v16-app``
...
- ``instruct`` — Display a series of steps using a notify box.
