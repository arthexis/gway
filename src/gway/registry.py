...
from .commands import TOP_LEVEL_COMMAND_NAMES
...
class Registry:
    ...
    def get(self, name_or_alias: str) -> Project | None:
        ...
        # Legacy registries could contain case-colliding names and aliases. Keep
        # exact spelling deterministic before falling back to folded lookup.
        ...
        if len(exact_alias_matches) > 1:
            raise RegistryError(f"ambiguous registered alias: {name_or_alias}")

        key = _identifier_key(name_or_alias)
        matches: list[Project] = []
        for name, record in records.items():
            if _identifier_key(name) == key or any(
                _identifier_key(alias) == key for alias in record.get("aliases", [])
            ):
                matches.append(Project.from_record(record))
        if len(matches) > 1:
            raise RegistryError(
                f"ambiguous case-insensitive project or alias: {name_or_alias}"
            )
    ...
    def _legacy_reserved_claims(
        ...
        allowed: set[str] = set()
        runtime_names = {_identifier_key(name) for name in TOP_LEVEL_COMMAND_NAMES}
