...
def _dummy_resume_state(
    *,
    has_previous_result: bool,
    previous_result: JSONValue,
    previous_result_provenance: ValueProvenance | None,
) -> ResumeCheckpoint:
    """Reuse the v1 validator to deep-copy JSON result/provenance state."""
    path = "pending-chain"
...
class PendingStageCheckpoint:
    """Portable description of one chain stage that has not executed yet."""
    ...
    def __post_init__(self) -> None:
        if isinstance(self.raw_tokens, (str, bytes, bytearray)) or not isinstance(
            self.raw_tokens, Sequence
        ):
    ...
    def from_stage(cls, stage: Stage) -> PendingStageCheckpoint:
        return cls(
            raw_tokens=stage.raw_tokens,
            kind=stage.kind,
            explicit_solve=stage.explicit_solve,
        )
    ...
    def as_dict(self) -> dict[str, JSONValue]:
        return {
            ...
            "explicit_solve": self.explicit_solve,
        }
    ...
    def from_dict(cls, raw: Mapping[str, object]) -> PendingStageCheckpoint:
        ...
        return cls(
            ...
            kind=kind,
            explicit_solve=_require_bool(
                raw["explicit_solve"], "pending stage.explicit_solve"
            ),
        )
...
class PendingChainCheckpoint:
    """Serializable continuation for one suspended multi-stage recipe statement."""
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.statement_tokens, (str, bytes, bytearray)) or not isinstance(
            self.statement_tokens, Sequence
        ):
            raise CheckpointError("pending chain.statement_tokens must be an array")
        statement_tokens = tuple(self.statement_tokens)
        if not statement_tokens or not all(
            isinstance(token, str) for token in statement_tokens
        ):
            raise CheckpointError("pending chain.statement_tokens must contain strings")

        active_stage_index = _require_int(
            self.active_stage_index, "pending chain.active_stage_index"
        )
        try:
        ...
        if active_stage_index < 1 or active_stage_index > len(parsed_stages):
            raise CheckpointError(
                "pending chain.active_stage_index must identify a stage in statement_tokens"
            )

        if isinstance(self.remaining_stages, (str, bytes, bytearray)) or not isinstance(
            self.remaining_stages, Sequence
        ):
        ...
        if not all(isinstance(stage, PendingStageCheckpoint) for stage in remaining_stages):
            raise CheckpointError(
                "pending chain.remaining_stages must contain pending stage checkpoints"
            )
        expected = tuple(
            PendingStageCheckpoint.from_stage(stage)
            for stage in parsed_stages[active_stage_index:]
        )
        if remaining_stages != expected:
            raise CheckpointError(
                "pending chain.remaining_stages does not match statement_tokens after the active stage"
            )

        has_previous_result = _require_bool(
            self.has_previous_result, "pending chain.has_previous_result"
        )
    ...
    def from_dict(cls, raw: Mapping[str, object]) -> PendingChainCheckpoint:
        ...
        return cls(
            ...
            recipe_path=_require_string(
                raw["recipe_path"], "pending chain.recipe_path"
            ),
            recipe_line=_require_int(raw["recipe_line"], "pending chain.recipe_line"),
            statement_tokens=_require_string_array(
                raw["statement_tokens"], "pending chain.statement_tokens"
            ),
            active_stage_index=_require_int(
                raw["active_stage_index"], "pending chain.active_stage_index"
            ),
            remaining_stages=tuple(
                PendingStageCheckpoint.from_dict(
                    _require_mapping(stage, f"pending chain.remaining_stages[{index}]")
                )
                for index, stage in enumerate(remaining_raw)
            ),
            has_previous_result=_require_bool(
                raw["has_previous_result"], "pending chain.has_previous_result"
            ),
            previous_result=previous_result,  # type: ignore[arg-type]
...
class ChainContinuationCheckpoint:
    """Version-3 checkpoint adding pending chain state to recipe continuation frames."""
    ...
    def __post_init__(self) -> None:
        if self.version != CHAIN_CONTINUATION_VERSION:
            raise CheckpointError(
                f"unsupported chain continuation version {self.version}; "
                f"expected {CHAIN_CONTINUATION_VERSION}"
            )

        stack = ContinuationStackCheckpoint(frames=self.frames, flags=self.flags)
        frames = tuple(stack.frames)
        if isinstance(self.pending_chains, (str, bytes, bytearray)) or not isinstance(
            self.pending_chains, Sequence
        ):
        ...
        for chain in pending_chains:
            if chain.frame_id in seen:
                raise CheckpointError(
                    f"duplicate pending chain for continuation frame: {chain.frame_id}"
                )
            seen.add(chain.frame_id)
            position = frame_positions.get(chain.frame_id)
            if position is None:
                raise CheckpointError(
                    f"pending chain references unknown continuation frame: {chain.frame_id}"
                )
            if position <= previous_position:
                raise CheckpointError(
                    "pending_chains must follow the same outer-to-inner order as frames"
                )
            ...
            if chain.recipe_path != frame.continuation.recipe_path:
                raise CheckpointError(
                    "pending chain recipe path does not match its continuation frame"
                )
            if chain.recipe_line != frame.continuation.line:
                raise CheckpointError(
                    "pending chain recipe line does not match its continuation frame"
                )
    ...
    def as_dict(self) -> dict[str, JSONValue]:
        return {
            ...
            "pending_chains": [chain.as_dict() for chain in self.pending_chains],
    ...
    def from_dict(cls, raw: Mapping[str, object]) -> ChainContinuationCheckpoint:
        _expect_keys(
            raw,
            {"version", "frames", "pending_chains", "flags"},
            "chain continuation checkpoint",
        )
        version = _require_int(raw["version"], "version")
        if version != CHAIN_CONTINUATION_VERSION:
            raise CheckpointError(
                f"unsupported chain continuation version {version}; "
                f"expected {CHAIN_CONTINUATION_VERSION}"
            )

        stack = ContinuationStackCheckpoint.from_dict(
            {
                "version": CONTINUATION_STACK_VERSION,
                "frames": raw["frames"],
                "flags": raw["flags"],
            }
        )
        ...
        pending_chains = tuple(
            PendingChainCheckpoint.from_dict(
                _require_mapping(chain, f"pending_chains[{index}]")
            )
            for index, chain in enumerate(pending_raw)
        )
        return cls(
            frames=stack.frames,
