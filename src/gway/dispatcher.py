...
class Dispatcher:
    ...
    def _resolve_default_command(
        ...
        tokens: Sequence[str],
    ) -> tuple[Command, list[str]] | None:
        command = next((item for item in commands if item.path == default_path), None)
        if command is None:
            return None
        return command, list(tokens)

    def commands(self, project_name: str) -> tuple[Command, ...]:
        return self._adapter(project_name).commands()

    def describe(self, project_name: str, path: tuple[str, ...]) -> Command:
        return self._adapter(project_name).describe(path)
    ...
    def run(
        ...
        *,
        ...
        tokens = list(argv)
        try:
            command, command_argv = self._resolve_command(commands, tokens)
        except CommandNotFound:
            default_path = project.default_command
            if default_path is None:
                raise
            resolved = self._resolve_default_command(commands, default_path, tokens)
            if resolved is None:
                raise
            command, command_argv = resolved
        command_argv = _decode_structured_argv(command, command_argv)
        if interactive:
            command_argv = _fill_required_options(command, command_argv)
        if isinstance(adapter, SigilContextAdapter):
            return adapter.run(command.path, command_argv, context=context)
        return adapter.run(command.path, command_argv)
    ...
    def capture_cli(
        ...
        *,
        ...
        """Capture the terminal values a normal managed CLI call would return."""
        project = self.registry.require(project_name)
        adapter = self._adapter(project_name)
        commands = adapter.commands()
        tokens = list(argv)
        ...
        command_argv = _decode_structured_argv(command, command_argv)
        if isinstance(adapter, SigilContextAdapter):
            result = adapter.run(command.path, command_argv, context=context)
        else:
            result = adapter.run(command.path, command_argv)
        return resolve_captured_cli_values(capture_cli_values(result))
