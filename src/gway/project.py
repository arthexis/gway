...
_ENVIRONMENT_PREFIX = re.compile(r"^[A-Za-z](?:[A-Za-z0-9_]*[A-Za-z0-9])?$")


def _environment_prefix(value: object) -> str | None:
    if value is None:
        return None
    if not isinstance(value, str) or not value.strip():
        raise ManifestError("[project].environment_prefix must be a non-empty string")
    normalized = value.strip().upper()
    if not _ENVIRONMENT_PREFIX.fullmatch(normalized):
        raise ManifestError(
            "[project].environment_prefix must contain only letters, numbers, and underscores, "
...
class Project:
    ...
    def __post_init__(self) -> None:
        ...
        if self.environment_prefix is not None:
            object.__setattr__(self, "environment_prefix", _environment_prefix(self.environment_prefix))
    ...
    def from_path(cls, path: str | Path) -> Project:
        ...
        environment_prefix = _environment_prefix(project_data.get("environment_prefix"))
    ...
    def from_record(cls, data: dict[str, Any]) -> Project:
        ...
        return cls(
            ...
            environment_prefix=data.get("environment_prefix"),
