...
def copy(value=None, *, notify=True, when=None):
    ...
    if value is not None:
        ...
        if notify:
            if abs(distance) <= 40:
                gw.studio.screen.notify(f"Clipboard modified: {value}")
            else:
                gw.studio.screen.notify("Clipboard modified (%+d bytes)" % distance)
    else:
    ...
    try:
        while True:
            ...
            if notify:
                ...
                gw.studio.screen.notify("Clipboard captured: " + summary)
