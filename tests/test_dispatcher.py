...
def test_dispatcher_resolves_project_aware_sigils(tmp_path: Path) -> None:
    ...
    result = dispatcher.run(
        "fx",
        ["peer", "add", "[project.name]", "[project.path]", "[command.path]", "[MODEL.name]"],
    )
    assert result == {
        "path": ("peer", "add"),
        "argv": ["fixture", str((tmp_path / "fixture").resolve()), "peer add", "demo"],
    }
...
def test_dispatcher_captures_eager_values_before_adapter_context(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result == {"path": ("peer", "add"), "argv": [str(Path.cwd()), "demo"]}
