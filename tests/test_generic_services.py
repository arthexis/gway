...
def test_service_install_preserves_target_arguments_after_separator(monkeypatch):
    ...
    result = runtime(
        "service install --backend process --attempts 5 -- worker --poll 2"
    )
...
def test_service_install_accepts_recipe_path(tmp_path, monkeypatch):
    ...
    result = runtime(f"service install --backend process -- {recipe}")
...
def test_named_service_identity_allows_multiple_invocations(monkeypatch):
    ...
    runtime("service install --backend process --name urgent -- worker --queue urgent")
    runtime("service install --backend process --name bulk -- worker --queue bulk")
